...
## 시간표 나머지 쓰기 — 수파베이스가 원본 (2026-08-26 완전 전환)
- **마이그레이션 없음.** 반 추가·반 삭제·반 통째 이동·1회 이동이 어댑터(timetable.html POSTS)에서
  수파베이스 먼저 처리된다 — 이로써 **시간표 쓰기 전부가 수파베이스 원본**.
- **반 추가**: 반ID를 페이지가 매긴다(백엔드와 같은 규칙 — 그 시간표의 `r###` 최대값+1,
  내신도 r 접두 그대로 1:1). unique(book,class_id) 충돌이면 다시 계산해 재시도.
  시트 사본은 같은 ID를 담아 뒤에서(`copy.id` — **백엔드 timetableAddClass가 id 파라미터
  수용하도록 수정, 리포트 @110 재배포**).
- **반 삭제**: 빈 반만(백엔드 동일 판정) + `roster=eq.''` 조건으로 그 사이 학생이 들어왔으면
  지우지 않는다.
- **반 통째 이동**: 요일·시간·위치·담당 PATCH + 짝 반(요일·시간만) + 정규면 소속 학생
  정규가/나 재계산(applySlots) — 전부 화면에서, 시트는 뒤에서.
- **1회 이동**: tt_log '1회' 기록 + **결석 보충 자동 완료 판정**(mkAutoComplete_ 1:1 —
  같은 반·같은 학생의 미완료 결석 중 적용일 ±14일 최근접)을 attendance(원본)에 직접.
  시트 사본(이동기록·출석기록 보충 열)은 백엔드가 같은 판정을 재수행.
- 넷 다 수파베이스 실패 시 옛 백엔드 폴백(어댑터 공통 흐름).
- **시트 사본 호출은 백그라운드라 도착 순서가 뒤섞일 수 있다** — 몇 초 안에 반 추가→학생
  추가→반 삭제처럼 연달아 하면 사본 쪽에서 순서가 어긋나 일부가 거절될 수 있다(원본은 정상,
  차이는 일일 점검이 보고). 실제 사용(작업 간 간격 수 초 이상)에선 드물다 — E2E 검증에서 확인.
- **일일 점검: tt_classes는 행·내용 모두 보고만**(heal=False). **시트 '정규시간표' 탭을 직접
  고쳐도 이제 반영되지 않는다** — 수정은 슈국 스케쥴 화면에서.
...
- **클리닉 신청 확인(shueguk-clinic teacher.html)**: 미러 먼저 그리던 화면을 이제 시트가 와도
  유지한다 — 시트는 사본 갱신용 행번호(_row)만 제공(제출시각·이름·시간대·유형·내용으로
  짝 잇기, sbClinicClearPatch와 같은 기준). **완료(클리어)도 원본 먼저**(2026-09-11 — 사용자
  "완료 버튼을 수파베이스 원본에 바로 기록"): 버튼이 원본 행 id(`_id`)로 `clinic_requests`를
...
## 모의고사 회차 평균 (023, 2026-08-30) — 학생 화면 성적 추이의 평균선
학생 페이지(s.html) 성적 추이에 **슈국 평균선**을 겹치려면 회차 평균이 필요한데, 학생 화면은
공개 키(anon)라 015 이후 `omr_responses`를 직접 읽지 못한다(읽히면 전교생 점수가 샌다).
그래서 **평균·응시 인원 숫자만** 돌려주는 함수 `omr_exam_stats(p)`를 따로 뒀다(anon 실행 허용).
- 입력 `{exams:[회차이름…]}` (학생이 응시한 회차만, 최대 30개) → 출력
  `{result:'success', stats:[{exam, subject, n, avg}]}`. subject 는 과목별 + 전체 합계 `'전체'`.
- 계산 규칙은 **교사 화면(hub `omr_analysis.html` 회차 분석)의 기본값과 같게** 맞췄다 —
  지금 정답으로 다시 채점(`omr_score_build_`), 이름·학교가 '테스트'인 제출 제외, 같은 학생
  (이름+학교)은 마지막 제출만. 그래서 두 화면의 평균이 어긋나지 않는다.
- **응시 5명 미만 회차는 avg 를 null 로 내보낸다** — 인원이 적으면 평균으로 남의 점수가 짐작된다.
  인원(n)은 그대로 준다. 화면은 그 회차에서 평균선을 끊는다.
...
- **030은 2026-09-20 실제 수파베이스에 적용 완료**(원장님이 SQL Editor에서 실행 — 교사 계정으로 select·PATCH 왕복 확인, 시험 값은 되돌림). 적용 전에도 페이지는 동작하게 돼 있다 — select가 400이면 옛 열로 다시 읽고 저장 때 kind를 빼서 보낸다.
...
## 학생정보 — 내용의 원본을 수파베이스로 (2026-08-26)
사용자 지시("학생정보가 시급")로 명단(roster) 전환과 같은 패턴을 적용했다.
- **읽기 전환**(superstar.html `SGETS`, timetable.html 신입 카드): `studentLinks`(슈스 링크)·
  `newStudents`(이달의 신입)·`roster`(자동완성 명단)를 `students` 표에서 직접 조립 —
  시트 통째 읽기(2~23초)가 0.2~0.5초로. 백엔드 응답과 **전수 대조: roster·newStudents 완전 일치,
  studentLinks는 6건 차이 — 전부 시트가 낡은 값**(지도중 중2 정규가, 8/23에 알려진 어긋남의 잔재.
  수파베이스가 실제 시간표와 일치). 그 6건은 updateStudent로 시트를 맞춰 두었다(전수 일치 확인).
- **쓰기 전환**(superstar.html `PRE`): `updateStudent`(슈스 링크 수정·등록일)·`studentRename`(표기 변경)은
  **수파베이스에 먼저 쓰고**(검증도 백엔드 규칙 1:1 — 학교·학년 필수, 등록일 형식, 표기 중복 거절)
  시트에는 같은 액션을 뒤에서 보낸다. 수파베이스 실패 시 옛 경로(백엔드 먼저 + AFTER 미러) 폴백.
  임시 학생으로 왕복 검증(생성→수정→없는 학생→중복 거절→표기 변경→삭제) 통과.
- **신입 등록(addStudent)·아웃(exitDelete)은 종전대로 백엔드 먼저 + 즉시 미러** — 접근코드 발급과
  행 삭제 판정이 백엔드에 있고, 미러가 즉시 갱신되므로 신선도 문제는 없다. 완전 전환하려면
  토큰 발급을 페이지로 옮기고 백엔드 addStudent가 받은 토큰을 쓰도록 고쳐 재배포해야 한다(다음 회차).
- **일일 점검**: students는 **행 추가·삭제만 시트 기준으로 맞추고 내용이 다르면 보고만** 한다
...
- `migrations/018_student_contacts.sql`: students에 `phone_student`(필수)·`phone_parent1`(필수)·
  ...
  (시트에는 종전 8자리 열만). student_bundle에는 노출 안 됨(me가 열을 골라 담아서).
- **신입 등록 폼이 전체 번호 입력으로** 바뀜: 학생·학부모1(필수)·학부모2(선택). 성적 확인
  비밀번호(학생ID)는 종전 규칙 그대로 **학부모1 번호의 뒤 8자리**를 페이지가 잘라 보낸다
  (백엔드 addStudent 계약 무변경). 등록 성공 후 전체 번호를 students에 PATCH(미러 삽입과 경합할 수
  있어 0행이면 0.9초 간격 재시도). 슈스 링크 [수정]에도 연락처 3칸 — PRE.updateStudent가 본 수정과
  **분리된 PATCH**로 저장(018 실행 전이어도 본 수정은 성공, 연락처만 경고).
- 초기 적재: `tools/import_contacts.py`(CRM '조건별 원생 조회' xlsx → 이름 매칭 → PATCH).
  예행 결과(2026-08-26): 496건 중 494 매칭(못 찾은 2 = 미등록생 손윤하·심채민), (모) 비면 (부)를
  학부모1로. **김준혁·심민교는 학부모1 뒤 8자리 ≠ 기존 학생ID** — 비밀번호(학생ID)는 건드리지
  않으므로 그대로지만, 번호가 바뀐 학생일 수 있으니 필요하면 확인.
- 일일 점검: 연락처 열은 시트에 없어 대조 대상 아님(soft_valf에 미포함) — 수파베이스가 유일 원본.
