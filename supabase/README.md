...
### ④ 잠그기 — `015_lock_anon.sql` (**아직 실행 안 함**)
anon 의 표 권한 회수 + 정책을 `to authenticated` 로. **배포 전에 실행하면 안 된다** —
지금 배포된(main) 페이지는 아직 공개 키로 표를 읽으므로 그 순간 멈춘다.
순서: 브랜치 머지 → 사이트 반영 확인 → 015 실행 → `tools/check_lock.py` 로 확인.
되돌리기 블록도 파일 안에 주석으로 있다.

`tools/check_lock.py` 는 세 가지를 본다 — 공개 키로 표 읽기(잠근 뒤 막혀야 함)·공개 키로
학생 함수 부르기(항상 되어야 함)·교사 신분으로 표 읽기(항상 되어야 함).
잠그기 전 실행 결과: 표 15/15 열림, 학생 함수 3/3 정상, 교사 신분 15/15 정상.
