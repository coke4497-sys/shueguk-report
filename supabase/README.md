...
### 다음 단계(사용자 확정 순서)
2. 외부 요청(UrlFetchApp) 권한 승인 — 어휘 편집기에서 `권한승인()` 1회. 서버끼리 미러가
   열려 OMR 응답 실시간 미러·어휘 지난 주차 차단이 살아난다.
3. 안전장치: 표별 접근 권한 분리 + 교사 페이지 인증 + DB→시트 일일 백업.
   **지금은 공개 키로 `students` 전체(학생ID=비밀번호, 접근코드, 전화)가 읽히고 모든 표가
   쓰기·삭제 가능하다(`anon_all`).** 쓰기를 옮기기 전에 반드시 선행.
4. 공지·배정·별 보너스 쓰기 → 5. 학생정보(신입·수정·퇴원 + 정규가/나) → 6. 정원·마감·채점.
