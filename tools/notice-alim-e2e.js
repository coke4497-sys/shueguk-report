 ...
 *      50명씩 나눠 alimSend(kind notice) 호출, 중복 키·변수, 숨김 공지는 안 보냄, 설정 전엔 체크 잠금
...
(async () => {
  ...
  ALIM_CFG = Object.assign({}, ALIM_CFG, { templates: { absent: ALIM_CFG.templates.absent, notice: Object.assign({}, ALIM_CFG.templates.notice, { ready: false }) } });
  await page.goto(NURL, { waitUntil: 'domcontentloaded' });
  await page.waitForFunction(() => /템플릿이 아직 없어요/.test(document.getElementById('alimNote').textContent), { timeout: 8000 });
  ok(await page.$eval('#alimOn', e => e.disabled), '공지 템플릿이 없으면 체크가 잠기고 안내');
