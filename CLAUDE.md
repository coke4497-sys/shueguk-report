...
- **교사 페이지**: `timetable.html`
  ...
  - **주차별 보기**: 월~일 달력 순서(월화수목금토일, 날짜 오름차순 — 2026-08-14 사용자 확정). 월·화 열은 그 주 수요일 '직전' 날짜(직전보강용), 수업 주차 자체는 수요일 시작. 지난주/다음주 이동. 그 주의 출석 기록·1회 이동이 반영된 주간 그리드(요일+날짜 헤더). 학생 클릭으로 해당 날짜 출석 소급 기록·수정 가능. 1회 이동 생성은 오늘의 시간표에서만.
